    ...
    *,
    ...
    try:
        with mlflow.start_run(run_name=run_name) as active_run:
            ...
            mlflow.log_param("train_years", "2010-2016")
            ...
            mlflow.log_metrics(
                {f"avg_gap_{county}": float(gap) for county, gap in mean_gap_by_county.items()}
            )
            ...
            if drift_report_path is not None:
                drift_report_path = Path(drift_report_path)
                if drift_report_path.is_file():
